  ...
  public:
    ...
    static void CompileAndRun(Json::Value req, Json::Value *resp)
    {
      ...
      if(!_Compile(file_header)) {
        ...
        std::string reason;
        FileUtil::ReadFile(CompileErrorPath(file_header), &reason);
        (*resp)["reason"] = reason;
        return;
      } 
      ...
      if(ret != 0) {
        ...
        (*resp)["reason"] = "program  exit by signal " + std::to_string(ret); 
        return;
      }
  ...
  private:
    ...
    static bool _Compile(const std::string file_header)
    {
      ...
      if(pid < 0) {
        ...
        int fd = open(CompileErrorPath(file_header).c_str(), O_CREAT | O_WRONLY,  0666);
        if(fd < 0) {
          perror("open");
          return false;
        }
